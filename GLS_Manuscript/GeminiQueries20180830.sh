...
## Define variables
WORKING_DIR='/mnt/causes-vnx2/TIDE/PROCESS/EXOME_TIDEX/T246_tmp/'
GEMINIDB='T246.db'
FAMILY_ID='T246'
...
rareEXONIC_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $EXONIC AND $SEGDUP AND $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE"  $GEMINIDB`
commonEXONIC_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $EXONIC AND $SEGDUP AND $GNOMAD_GENOME_COMMON AND $GNOMAD_EXOME_COMMON"  $GEMINIDB`
...
rareCODING_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $CODING AND $SEGDUP AND $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE"  $GEMINIDB`
commonCODING_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $CODING AND $SEGDUP AND $GNOMAD_GENOME_COMMON AND $GNOMAD_EXOME_COMMON"  $GEMINIDB`
...
rareSPLICING_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $SPLICING AND $SEGDUP AND $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE"  $GEMINIDB`
commonSPLICING_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $SPLICING AND $SEGDUP AND $GNOMAD_GENOME_COMMON AND $GNOMAD_EXOME_COMMON"  $GEMINIDB`
...
rareLOF_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $LOF AND $SEGDUP AND $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE"  $GEMINIDB`
commonLOF_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $LOF AND $SEGDUP AND $GNOMAD_GENOME_COMMON AND $GNOMAD_EXOME_COMMON"  $GEMINIDB`
...
rareGENEHANCER_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $GENEHANCER AND $SEGDUP AND $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE"  $GEMINIDB`
commonGENEHANCER_VARS=`gemini query -q "SELECT COUNT(*) FROM variants \
	WHERE $GENEHANCER AND $SEGDUP AND $GNOMAD_GENOME_COMMON AND $GNOMAD_EXOME_COMMON"  $GEMINIDB`
...
gemini query -q "SELECT $COLUMNS FROM variants WHERE ( (clinvar_pathogenic == 'Pathogenic') OR (clinvar_pathogenic == 'Likely Pathogenic') ) AND (clinvar_disease_name is not NULL) AND $CONFIDENTREGION AND $SEGDUP AND $FILTER" \
...
gemini query -q "SELECT $COLUMNS FROM variants WHERE $GNOMAD_GENOME_RARE AND $GNOMAD_EXOME_RARE AND $INHOUSE_RARE AND ($IMPACT_HIGH OR $IMPACT_MED) AND $CONFIDENTREGION AND $SEGDUP AND $FILTER" \
