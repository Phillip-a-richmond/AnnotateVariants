...
## Split SV files
SVFILES=$VCFDIR/*-smoove.genotyped.vcf.gz
...
cd $VCFDIR
ls *-smoove.genotyped.EPGEN*.vcf > Sample_SV_VCFs_list_${DateOfCreation}.txt
...
$SURVIVOR merge \
	 ...
	 0 \
	 1 \
	 InHouseDB_SV_50_${DateOfCreation}.vcf
...
$SURVIVOR vcftobed \
	...
	1 \
	1000000000 \
	InHouseDB_SV_50_${DateOfCreation}.bedpe



# Step 4 - Add frequency to bedpe
python $BUILDINHOUSEDBDIR/AddFreqToBedpe.py -V InHouseDB_SV_50_${DateOfCreation}.vcf \
	-B InHouseDB_SV_50_${DateOfCreation}.bedpe \
	-O InHouseDB_SV_50_${DateOfCreation}.freq.bedpe


# Step 5 - Cut out columns and make a bed file
grep -v 'TRA' InHouseDB_SV_50_${DateOfCreation}.freq.bedpe | \
        cut -f1,2,6,7,11,12,13 - \
        > InHouseDB_SV_50_${DateOfCreation}.bed

# Step 6 - add SV name 
python $BUILDINHOUSEDBDIR/add_SV_name.py \
	InHouseDB_SV_50_${DateOfCreation}.bed \
	InHouseDB_SV_50_${DateOfCreation}.final.bed


# Step 7 - add header line, cut-out freq and af columns from final bed, concatenate with header
## Frequency 
echo "#chrom	start	end	InHouseDB_SV_Freq" > InHouseDB_SV_Freq.header
cut -f1,2,3,7  InHouseDB_SV_50_${DateOfCreation}.final.bed | cat InHouseDB_SV_Freq.header - > UserInHouseDB_SV_${DateOfCreation}.Freq.bed


## Count
echo "#chrom	start	end	InHouseDB_SV_Count" > InHouseDB_SV_Count.header
cut -f1,2,3,6  InHouseDB_SV_50_${DateOfCreation}.final.bed | cat InHouseDB_SV_Count.header - > UserInHouseDB_SV_${DateOfCreation}.Count.bed
