...
## Unlike the hard coded trio script, this script will perform variant calling separately for each case generating GVCF files, and then joint genotype over them.
...
# Check your assumed samples
if [ "$SIBLING_PRESENT" = true ] ; then
	echo "Sibling is present"
fi

if [ "$MOTHER_PRESENT" = true ] ; then
        echo "Mother is present"
fi

if [ "$FATHER_PRESENT" = true ] ; then
        echo "Father is present"
fi
...
## IF there is a sibling, add it as such:
#	--sibling ${SIBLING_ID}_$GENOME_BUILD,$SIBLING_SEX,$SIBLING_AFFECTED \

python $PEDMAKER \
        --proband ${PROBAND_ID}_$GENOME_BUILD,$PROBAND_SEX,affected \
        --father ${FATHER_ID}_$GENOME_BUILD,male,$FATHER_AFFECTED \
        --mother ${MOTHER_ID}_$GENOME_BUILD,female,$MOTHER_AFFECTED \
        -F $FAMILY_ID \
        -O $PED
...
if [ "$SIBLING_PRESENT" = true ] ; then
	...
	## Step 2 Sibling
	### Copy to scripts directory
	cp $TEMPLATE_DIR/$STEP2_TEMPLATE ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	
	### Make changes with sed to replace values, starting with Change ID
	sed -i "s/sample/$SIBLING_ID/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/working_dir/$WORKING_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/raw_dir/$RAW_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/email_address/$EMAIL/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/annotate_variants_dir/$ANNOTATE_VARIANTS_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/fasta_dir/$FASTA_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/fasta_file/$FASTA_FILE/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/genome_build/$GENOME_BUILD/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/fastqr1/$SIBLING_FASTQR1/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
	sed -i "s/fastqr2/$SIBLING_FASTQR2/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP2_TEMPLATE}
...
# DeepVariant --> GLNexus: Joint Genotype to produce SNV/indel
STEP3_TEMPLATE=RunDeepVariant_Template.sh
...
sed -i "s/sibling_boolean/$SIBLING_PRESENT/g" ${WORKING_DIR}/${FAMILY_ID}_${STEP3_TEMPLATE}
sed -i "s/mother_boolean/$MOTHER_PRESENT/g" ${WORKING_DIR}/${FAMILY_ID}_${STEP3_TEMPLATE}
sed -i "s/father_boolean/$FATHER_PRESENT/g" ${WORKING_DIR}/${FAMILY_ID}_${STEP3_TEMPLATE}
sed -i "s/sibling_id/$SIBLING_ID/g" ${WORKING_DIR}/${FAMILY_ID}_${STEP3_TEMPLATE}
...
if [ "$SIBLING_PRESENT" = true ] ; then
	## Sibling
	### Copy the template
	cp ${TEMPLATE_DIR}/$STEP4_TEMPLATE ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	
	### Edit the template
	sed -i "s/annotate_variants_dir/$ANNOTATE_VARIANTS_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/working_dir/$WORKING_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/raw_dir/$RAW_DIR_VAR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/email_address/$EMAIL/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/sample_id/$SIBLING_ID/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/mtoolbox_dir/$MTOOLBOX_DIR/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/family_id/$FAMILY_ID/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/fastqr1/$SIBLING_FASTQR1/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
	sed -i "s/fastqr2/$SIBLING_FASTQR2/g" ${WORKING_DIR}/${SIBLING_ID}_${STEP4_TEMPLATE}
