...
# Something to note, is that the SAMPLE ID now has the version of the pipeline you ran
...
if len(sys.argv) < 2:
	...
	print "python Pipeline_Master_WGS.py -v new -d /mnt/causes-data04/ -s TDX-468 -1 /mnt/causes-data04/data2/RAW/EXOME_TIDEX/Family121/TIDEX468_CAGATC_L001_R1_001.fastq.gz -2 /mnt/causes-data04/data2/RAW/EXOME_TIDEX/Family121/TIDEX468_CAGATC_L001_R1_001.fastq.gz -p 16 -m 100G"
	sys.exit()


# Redone with Argparse
parser = argparse.ArgumentParser()
parser.add_argument("-v","--version",help="Choose which pipeline version: old or new\nold: Bowtie2-Samtools\nnew:BWA-Platypus\nIf you put down 'neither', then it will not run the primary pipeline")
parser.add_argument("-d","--workingDir",help="Working directory on your filesystem")
parser.add_argument("-s","--sampleID",help="What is the sample ID for these input files")
parser.add_argument("-p","--processors",help="Choose the number of processors for this job",type=int)
parser.add_argument("-m","--memory",help="Choose the memory needed for this job")
parser.add_argument("-1","--inputR1",help="The R1 fastq file")
parser.add_argument("-2","--inputR2",help="The R2 fastq file")
parser.add_argument("-n","--nodeName",help="Name of the node you want to submit to, options are: cfricauseshpc02,cfricauseshpc03,cfricauseshpc04,cfricauseshpc06")
parser.add_argument("-a","--annotate",help="Produce annotation header",action='store_true',default=False)
...
# This masked argument is no longer functional since I'm maintaining both hg19 and GSC genomes
# Plus it wasn't ever really useful
...
# Adding in a header file to be added to /mnt/data/PipelineControl/processingpipeline/AnnotationPipeline_Template.sh, and that'll be the final annotation pipeline to be run, I'm making the file, but it won't be a part of anything that gets qsubbed.  Only run this for the child

if args.annotate:
	HeaderForAnnoFile = open("%s%s_AnnotationHeader.txt"%(workingDir,sampleID),'w')
	HeaderForAnnoFile.write("FAMILY_ID='%s'\n"%sampleID[:-2])
	HeaderForAnnoFile.write("SAMPLE_ID_1='%s-1'\n"%sampleID[:-2])
	...
	if args.version == 'new':
		...
		HeaderForAnnoFile.write("BAM3='%s%s-3_BWAmem_dupremoved_realigned.sorted.bam'\n"%(workingDir,sampleID[:-2]))
	elif args.version == 'old':
		HeaderForAnnoFile.write("BAM1='%s%s-1_bowtie2_dupremoved_realigned.sorted.bam'\n"%(workingDir,sampleID[:-2]))
...
# Added in the -@ $NSLOTS for parallelization of samtools commands
def Sam2Bam():
...
#SNP calling
	...
	shellScriptFile.write("	-nct 1 --never_trim_vcf_format_field \\\n")
...
# Splitting out SV from CNV, improve speed
if args.sv:
	if not args.cnv:
		CNVNATOR()
	Lumpy()
