...
def GetOptions():
    ...
    parser.add_argument("-C","--Column",help="Column (zero-based) where the gene name(s) is listed.",type=int,required=True)
    args = parser.parse_args()
    infilename=args.InFile
    outfilename=args.OutFile
    GeneCol=args.Column
    return infilename,outfilename,GeneCol
...
def AddColumnsToTable(GeminiInFileName,GeminiOutFileName,Gene2Pheno,Gene2Mim,GeneSummary,Gene2PLI,Gene2RVIS,Gene2HPO,Gene2MESHOP,GeneCol):
	...
	for line in infile:
		line = line.strip('\n')
		cols = line.split("\t")
		genes = cols[GeneCol].split(';')
	
		# because multiple genes may be present, split by ';'
		# I will break each of these annotations into an array, loop through the genes, append to each array, then collapse
		omim_phenos = []
		omim_hyperlinks = []
		hpos = []
		pLI_scores = []
		...
		for gene in genes:
			...
			# add to array
			omim_phenos.append(omim_pheno)
			omim_hyperlinks.append(omim_hyperlink)
			hpos.append(hpo)
			pLI_scores.append(pLI_score)
			pLI_pcts.append(pLI_pct)
			rvis_scores.append(rvis_score)
			rvis_pcts.append(rvis_pct)
			meshops.append(meshop)
			gene_summarys.append(gene_summary)		


		# collapse by ';'
		omim_pheno=';'.join(omim_phenos)	
		omim_hyperlink=';'.join(omim_hyperlinks)	
		hpo=';'.join(hpos)	
		pLI_score=';'.join(pLI_scores)	
		pLI_pct=';'.join(pLI_pcts)	
		rvis_score=';'.join(rvis_scores)	
		rvis_pct=';'.join(rvis_pcts)	
		meshop=';'.join(meshops)	
		gene_summary=';'.join(gene_summarys)	
...
# They contain gene-based information
    ...
    GeminiInfile,GeminiOutfile,GeneCol=GetOptions()
    ...
    AddColumnsToTable(GeminiInfile,GeminiOutfile,Gene2Pheno,Gene2Mim,GeneSummaries,Gene2PLI,Gene2RVIS,Gene2HPO,Gene2MESHOP,GeneCol)
